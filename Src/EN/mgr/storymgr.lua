...
function slot0.Play(slot0, slot1, slot2, slot3, slot4, slot5)
	...
	if getProxy(ChapterProxy) and slot6:getActiveChapter() and PlayerPrefs.GetInt("skip_events_on_" .. slot8.id, 1) == 0 then
		slot7 = true
	end
	...
	slot9 = (slot2 and true) or slot1[slot1].type ~= slot3
...
function slot0.StartStory(slot0, slot1, slot2, slot3, slot4)
	...
	coroutine.wrap(function ()
		for slot3, slot4 in ipairs(slot0.scripts) do
			...
			if slot1 then
				break
			elseif slot4.optionFlag and slot2.optionFlag ~= slot4.optionFlag then
			else
				slot5 = true

				setActive(slot2._flash, false)

				slot2.blackSkip = false
				slot6 = slot4.mode or slot0.mode

				if slot6 == slot3.ASIDE then
					if slot4.flashout then
						slot2:flashout(slot4, function ()
				...
				if slot4.options then
					if not slot4.typewriter then
						slot2:initOptions(slot4, function ()
							slot0()
						end)
					else
						function slot2.typeWriteCallback()
				...
				if slot4.effects then
					slot2:setEffects(slot4.effects)
				end

				SetActive(slot2.nextTF, true)
				...
				if slot4.flash then
					slot2._flash:GetComponent(typeof(Image)).color = (slot4.flash.black and Color(0, 0, 0)) or Color(1, 1, 1)
					slot2._flashCG.alpha = slot4.flash.alpha[1]

					setActive(slot2._flash, true)

					for slot11 = 1, slot4.flash.number, 1 do
						slot12 = LeanTween.value
						slot13 = go(slot2._flash)
				...
				if slot4.flashN then
					slot2._flash:GetComponent(typeof(Image)).color = (slot4.flashN.color and Color(slot4.flashN.color[1], slot4.flashN.color[2], slot4.flashN.color[3])) or Color(0, 0, 0)

					setActive(slot2._flash, true)

					for slot11, slot12 in ipairs(slot4.flashN.alpha) do
						slot14 = LeanTween.value(go(slot2._flash), slot12[1], slot12[2], slot12[3])
						slot13 = LeanTween.value(go(slot2._flash), slot12[1], slot12[2], slot12[3]).setDelay
				...
				if slot4.dialogShake then
					slot2.interactive = false

					LeanTween.moveX(slot2._dialogue, slot4.dialogShake.x, slot4.dialogShake.speed):setLoopPingPong(slot4.dialogShake.number):setOnComplete(System.Action(function ()
						slot0.interactive = true
				...
				if type(slot4.jumpto) == "string" then
					slot2.nextEpisode = slot4.jumpto
		...
		if not slot1 and slot0.fadeOut then
			if slot0.noWaitFade and slot5 then
				slot5()
				slot2:fadeOut(slot0.fadeType, slot0.fadeOut)
			else
				slot2:fadeOut(slot0.fadeType, slot0.fadeOut, slot5)
			end

			slot1 = true
		else
			if not slot1 and slot0.occlusion then
				slot2.occlusion = true
				slot2.blackScreenTimer = Timer.New(function ()
					slot0:EndStory()
				...
				slot2.blackScreenTimer:Start()

				slot1 = true
			else
				slot2:EndStory(slot5)
			end
		end
	end)()
end
...
function slot0.showOptions(slot0)
	...
	LeanTween.delayedCall(slot1 * 0.1, System.Action(function ()
		slot0.optionsCg.interactable = true
...
function slot0.setEffects(slot0, slot1)
	for slot5, slot6 in pairs(slot1) do
		if slot6.name then
			if not findTF(slot0._effectPanel, slot6.name) then
				if not slot0.effectObjs[slot6.name] then
					PoolMgr.GetInstance():GetUI("speed", false, function (slot0)
						setParent(slot0, slot0._effectPanel.transform)
						setActive(slot0, slot1.active)

						slot0.effectObjs[slot1.name] = {}
						slot0.effectObjs[slot1.name][1] = slot1.name
						slot0.effectObjs[slot1.name][2] = slot0

						return
					end)
				else
					setActive(slot8[2], slot6.active)
				end
			else
				SetActive(slot7, slot6.active)
			end
		end
	end
