				...
				for slot9, slot10 in ipairs(slot1) do
					...
					if slot9 == 1 then
						...
						if slot0:getConfig("config_id") == 2 or slot0:getConfig("config_id") == 3 then
							slot12 = slot1:getActivityTask(slot0)
							slot13 = slot1:getTaskById(slot12)
							slot11.showTips = (slot12 and not slot13 and not slot1:getFinishTaskById(slot12)) or (slot13 and slot13:isFinish())
						else
							if slot1:getAcademyTask(slot0[slot10.id].groupId) then
								if slot1:getTaskById(slot12) or slot1:getFinishTaskById(slot12) then
									slot5 = slot13
								else
									slot4 = slot12
								end
							...
							slot11.showTips = slot4 ~= nil or (slot5 and slot5:getTaskStatus() == 1)
							slot11.acceptTaskId = slot4
							slot11.currentTask = slot5
						end

						slot2[slot10.id] = slot11
					end

					if slot10.tasks then
						slot12 = true
						slot15 = ((table.indexof(slot3, slot4) or 0) > 0 and (table.indexof(slot3, slot4) or 0)) or (slot5 and table.indexof(slot3, slot5.id)) or 0
	...
	for slot6, slot7 in pairs(slot1) do
		if not slot0.academyStudents[slot6] then
			...
			slot10:setCallBack(function (slot0)
				...
				slot0:onTask(slot1, slot3[slot2])
