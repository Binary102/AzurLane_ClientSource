...
function slot0.initEquipment(slot0)
	...
	for slot4, slot5 in ipairs(slot0.equipmentPanels) do
		...
		table.insert(slot0.equipmentIcons, slot0:findTF("info/icon_bg", slot5))
	end
end
				...
				elseif slot15 == true and slot19 == 4 then
					...
					if slot14[slot19] then
						...
						if slot2.config.type ~= EquipType.Equipment and slot23.type == AttributeType.Reload and slot19 == 4 then
							...
							setText(slot25, setColorStr(slot27 .. "s", COLOR_YELLOW))
						else
		...
		for slot6, slot7 in ipairs(slot0.fashionSkins) do
			...
			onButton(slot0, slot9, function ()
				...
				if HXSet.isHx() then
					...
					slot0:loadPainting((slot2.painting_hx ~= "" and slot2.painting_hx) or slot2.painting)
				end
...
function slot0.updateFashionDetail(slot0, slot1)
	...
	onButton(slot0, slot2.confirm, function ()
		if slot0 then
		elseif slot1 then
			slot2:emit(slot2, slot3, (slot3.id == slot2.shipVO:getConfig("skin_id") and 0) or slot3.id)
		elseif slot4 then
			if slot5 then
...
function slot0.didEnter(slot0)
	...
	onButton(slot0, slot0.helpBtn, function ()
		...
		elseif slot0.page == slot1.PAGE.DETAIL then
			pg.MsgboxMgr.GetInstance():ShowHelpWindow({
				helps = pg.gametip.help_shipinfo_detail.tip
			})
		elseif slot0.page == slot1.PAGE.INTENSIFY then
			pg.MsgboxMgr.GetInstance():ShowHelpWindow({
				helps = pg.gametip.help_shipinfo_intensify.tip
			})
		elseif slot0.page == slot1.PAGE.UPGRADE then
			pg.MsgboxMgr.GetInstance():ShowHelpWindow({
				helps = pg.gametip.help_shipinfo_upgrate.tip
			})
		elseif slot0.page == slot1.PAGE.FASHION then
			pg.MsgboxMgr.GetInstance():ShowHelpWindow({
	...
	onButton(slot0, slot0.evaluationBtn, function ()
		slot0:emit(ShipInfoMediator.OPEN_EVALUATION, slot0.shipVO:getGroupId())
	end, SFX_PANEL)
...
function slot0.switchToPage(slot0, slot1)
	...
	elseif slot1 == slot0.PAGE.REMOULD then
		slot0:switchPanel(slot0.shipInfo, -slot2, nil, slot1 * 2)
		...
		elseif slot1 == slot0.PAGE.FASHION then
	...
	if slot0.page == slot0.PAGE.FASHION then
		slot0:updateFashion()
	elseif slot0.page == slot0.PAGE.DETAIL then
		slot0:checkMaxLevelHelp()
	end

	setActive(slot0.npcFlagTF, slot0.page ~= slot0.PAGE.UPGRADE and slot0.shipVO:isActivityNpc())
end
...
function slot0.moveUIPanels(slot0, slot1, slot2)
	...
	elseif slot1 == slot0.PAGE.INTENSIFY and slot0.modPanel then
		setAnchoredPosition(slot0.modPanel, {
			x = 620 + slot2
		})
	elseif slot1 == slot0.PAGE.DETAIL then
		setAnchoredPosition(slot0.detailPanel, {
			x = slot2
		})
	elseif slot1 == slot0.PAGE.FASHION then
...
function slot0.updateRecordEquipments(slot0, slot1)
	...
	for slot8 = 1, 5, 1 do
		...
		if slot10 then
			...
			slot16 = (slot0.shipVO.equipments[slot8] and slot15.id == slot9) or false

			setActive(slot13:Find("tip"), not ((slot0.shipVO.equipments[slot8] and slot15.id == slot9) or false) and (not slot14 or slot14.count <= 0))
			updateEquipment(slot13, Equipment.New({
				id = slot9
			}))

			if not ((slot0.shipVO.equipments[slot8] and slot15.id == slot9) or false) and (not slot14 or slot14.count <= 0) then
...
function slot0.switch2EquipmentSkinPage(slot0)
	...
	for slot4, slot5 in pairs(slot0.equipmentIcons) do
	end
