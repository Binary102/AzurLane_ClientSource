...
function slot0.updateTraningCampBtn(slot0)
	...
	slot5 = (slot1 ~= nil and slot1.data1) or 0
	slot7 = getProxy(TaskProxy):getTaskById(13873) or slot6:getFinishTaskById(13873)

	if slot5 >= 1 and slot7 ~= nil and slot7:isReceive() then
		setActive(slot0.traingCampBtn, false)
	end
end
	...
	if not slot0.live2dCom or slot0.live2dCom.name ~= slot1 then
		pg.Live2DMgr.GetInstance():GetLive2DModelAsync(slot1, function (slot0)
			...
			if CSharpVersion > 18 then
				slot6 = pg.ship_skin_template[slot0.flagShip.skinId].lip_smoothing

				if pg.ship_skin_template[slot0.flagShip.skinId].lip_sync_gain and slot5 ~= 0 then
					slot0:findTF("paint/live2d"):GetChild(0):GetComponent("CubismCriSrcMouthInput").Gain = slot5
				end

				if slot6 and slot6 ~= 0 then
					slot0:findTF("paint/live2d"):GetChild(0):GetComponent("CubismCriSrcMouthInput").Smoothing = slot6
				end
			end
...
function slot0.setFlagShip(slot0, slot1)
	...
	if slot0.Live2dChar ~= slot1:getPainting() then
		slot0:ShowAssistInfo(slot2, slot1:getLive2dOffset())
	end
