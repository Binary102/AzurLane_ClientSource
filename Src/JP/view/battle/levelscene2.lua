...
function slot0.updateCouldAnimator(slot0)
	if slot0.contextData.map:getConfig("animtor") == 1 then
		...
		slot0.mapAnimPresentOver = false
		...
		if IsNil(slot0.tornadoTF) then
			...
			PoolMgr.GetInstance():GetUI(slot1, true, function (slot0)
				...
				if slot0.tornadoTF:GetComponent("DftAniEvent") then
					slot2:SetEndEvent(function (slot0)
						slot0.mapAnimPresentOver = true
					end)
				end
			end)
		else
...
function slot0.registerActBtn(slot0)
	...
	onButton(slot0, slot0.actNormalBtn, function ()
		...
		if slot1:isfrozen() then
			return
		end

		if slot1.contextData.map:isRemaster() then
			slot1:setMap(slot0:getBindMap())
		...
		if not slot2(Map.ACTIVITY_EASY, slot1.battleActivity.id) then
			pg.TipsMgr:GetInstance():ShowTips(i18n("battle_levelScene_lock"))

			return
		end

		slot1:emit(LevelMediator2.ON_SWITCH_ACT_MAP, slot1.battleActivity.id, Map.ACTIVITY_EASY)
	end, SFX_PANEL)
end
...
function slot0.updateEscortItem(slot0, slot1, slot2, slot3)
	...
	for slot22 = 0, slot14:GetComponentsInChildren(typeof(Image)).Length - 1, 1 do
		slot18[slot22].color = slot17
	end

	setImageColor(slot1, slot17)
	onButton(slot0, slot1, function ()
		if slot0 then
...
function slot0.willExit(slot0)
	...
	if getProxy(ChapterProxy) and not slot0.contextData.map:isActivity() and not slot2:isSham() and not slot2:isGuildBoss() and not slot2:isEscort() then
