...
function slot0.didEnter(slot0)
	...
	if "" == AUDIT_SERVER_ID then
		SetActive(slot0.togglePanel:GetChild(0), false)
	end

	for slot5 = 1, 4, 1 do
		table.insert(slot0.toggles, slot6)
		onToggle(slot0, slot0.togglePanel:GetChild(slot5 - 1), function (slot0)
			if slot0 then
	...
	slot2 = slot0.contextData.wrap or slot0.TYPE_DIAMOND

	if slot1 == AUDIT_SERVER_ID and slot2 == slot0.TYPE_SKIN then
		slot2 = slot0.TYPE_DIAMOND
	end

	triggerToggle(slot0.toggles[slot2], true)
	slot0:updateNoRes()
end
...
function slot0.updateSkinPage(slot0)
	slot1 = {}
	slot3 = "" ~= AUDIT_SERVER_ID

	for slot7, slot8 in ipairs(pg.shop_template.all) do
		if pg.shop_template[slot8].genre == "skin_shop" then
			slot9 = slot3 or pg.shop_template[slot8].showInAudit ~= 1
			slot10 = true

			if HXSet.isHx() then
				slot10 = pg.shop_template[slot8].isHX ~= 1
			end

			if slot9 and slot10 then
				slot11:updateBuyCount((table.contains(slot0.skinList, Goods.New({
					shop_id = slot8
				}, Goods.TYPE_SKIN).getConfig(slot11, "effect_args")[1]) and 1) or 0)

				slot14, slot15 = pg.TimeMgr.GetInstance():inTime(pg.shop_template[slot8].time)

				if slot15 then
					slot0:addUpdateTimer(slot15)
				end

				if slot14 then
					if #slot1 == 58 then
						print(slot11.skinId)
						print(slot11.configId)
					end

					table.insert(slot1, slot11)

					slot16, slot17 = pg.TimeMgr.GetInstance():inTime(slot11:getConfig("discount_time"))

					if slot17 then
						slot0:addUpdateTimer(slot17)
					end
				end
			end
		end
	...
	for slot8, slot9 in ipairs(pg.activity_shop_template.all) do
		if pg.activity_shop_template[slot9].commodity_type == DROP_TYPE_SKIN and slot4:getActivityById(slot10.activity) and not slot11:isEnd() then
			slot12:updateBuyCount((table.contains(slot0.skinList, Goods.New({
				shop_id = slot9
			}, Goods.TYPE_ACTIVITY).getConfig(slot12, "commodity_id")) and 1) or 0)
			table.insert(slot1, slot12)
		end
	end

	for slot8, slot9 in ipairs(pg.activity_shop_extra.all) do
		if pg.activity_shop_extra[slot9].commodity_type == DROP_TYPE_SKIN then
			slot11 = slot4:getActivityById(slot10.activity)

			if (slot10.activity == 0 and pg.TimeMgr.GetInstance():inTime(slot10.time)) or (slot11 and not slot11:isEnd()) then
				slot12:updateBuyCount((table.contains(slot0.skinList, Goods.New({
					shop_id = slot9
				}, Goods.TYPE_ACTIVITY_EXTRA).getConfig(slot12, "commodity_id")) and 1) or 0)
				table.insert(slot1, slot12)
			end
		end
	end

	slot5 = slot0.contextData.skinSortId or 0

	for slot9 = #slot1, 1, -1 do
		print(slot9)

		if pg.ship_skin_template[slot1[slot9]:getSkinId()].shop_type_id ~= slot5 and slot5 ~= 0 and (slot11.shop_type_id ~= 0 or slot5 ~= 9999) then
