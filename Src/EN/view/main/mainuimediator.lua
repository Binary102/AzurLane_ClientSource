...
slot0.OPEN_NOTICE = "MainUIMediator.OPEN_NOTICE"
...
slot0.OPEN_COLORING = "MainUIMediator:OPEN_COLORING"
...
slot0.ON_ACTIVITY1 = "MainUIMediator:ON_ACTIVITY1"
...
function slot0.register(slot0)
	...
	slot0:bind(slot0.OPEN_CHUANWUSTART, function (slot0, slot1)
		...
		elseif slot1 == DockyardScene.MODE_UPGRADE then
			slot0:sendNotification(GAME.GO_SCENE, SCENE.DOCKYARD, {
				prevFlag = false,
				mode = slot1,
	...
	slot0:bind(slot0.OPEN_RANK, function (slot0)
		...
		slot0:sendNotification(GAME.GO_SCENE, SCENE.BILLBOARD, {
			index = BillboardLayer.PAGE_POWER
		})
	end)
	...
	slot0:bind(slot0.OPEN_NOTICE, function (slot0)
		if #getProxy(ServerNoticeProxy).getServerNotices(slot1, false) > 0 or isAiriUS() then
			slot0:addSubLayers(Context.New({
				mediator = ServerNoticeMediator,
				viewComponent = ServerNoticeLayer
			}), nil, function ()
	...
	slot0:bind(slot0.OPEN_COLORING, function (slot0)
		slot0:sendNotification(GAME.GO_SCENE, SCENE.COLORING)
	end)
end
...
function slot0.ActiveOpera()
	if #getProxy(ActivityProxy).getActivitiesByType(slot0, ActivityConst.ACTIVITY_TYPE_ZPROJECT) > 1 then
		pg.m02:sendNotification(GAME.GO_SCENE, SCENE.LEVEL, {
			chapterId = getProxy(ChapterProxy):getUnlockActMapByActs(slot1).getActiveChapter(slot2) and slot3.id,
			mapIdx = slot2.id
		})
	elseif slot0:getActivityByType(ActivityConst.ACTIVITY_TYPE_ZPROJECT) and not slot2:isEnd() then
		if ActivityConst.BATTLE_MAP_TYPE == Map.SINGLE_MAP then
			pg.m02:sendNotification(GAME.GO_SCENE, SCENE.LEVEL, {
				chapterId = getProxy(ChapterProxy):getActActiveChapter() and slot3.id,
				mapIdx = (slot3 and slot3:getConfig("map")) or ActivityConst.ACTIVITY_BATTLE_MAP_ID
			})
		elseif ActivityConst.BATTLE_MAP_TYPE == Map.MUTIL_MAP then
			if slot2:getData1() == 0 then
				if ActivityConst.SELECT_MAP_DIFFICULT then
					pg.m02:sendNotification(GAME.LOAD_LAYERS, {
						parentContext = getProxy(ContextProxy):getCurrentContext(),
						context = Context.New({
							viewComponent = LevelDifficultySelLayer,
							mediator = LevelDifficultySelMediator,
							data = {
								activityId = slot2.id
							}
						})
					...
					pg.m02:sendNotification(GAME.ACTIVITY_OPERATION, {
						...
						arg1 = Map.ACTIVITY_EASY
					})
				end
			else
				pg.m02:sendNotification(GAME.GO_SCENE, SCENE.LEVEL, {
					chapterId = getProxy(ChapterProxy):getUnlockActMapBytype(slot3, slot2.id).getActiveChapter(slot4) and slot5.id,
					mapIdx = slot4.id
				})
			end
		end
...
function slot0.handleEnterMainUI(slot0)
	...
	if pg.GuideMgr2.ENABLE_GUIDE == false then
		slot1 = nil
