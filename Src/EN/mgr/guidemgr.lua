...
function pg.GuideMgr.play(slot0, slot1, slot2, slot3)
	...
	print("play guide >>", slot1, isPlay)

	slot4, slot5 = slot0:canPlay()
	...
	if slot4 then
		...
		slot6 = Clone(slot0.currentGuide.events)

		if slot2 then
			slot0.curEvents = _.select(slot6, function (slot0)
	...
	if slot1.style then
		...
		if slot1.style.ui then
			slot0.finder:Search({
				...
				found = function (slot0)
					slot0.cloneTarget = slot0:cloneGO(go(slot0), slot0.UIOverlay, slot1.style.ui)
				end,
				notFound = function ()
					slot0:endGuider()
				end
...
function pg.GuideMgr.updateContent(slot0, slot1)
	...
	setAnchoredPosition(slot0.guiderTF, Vector3(slot5, slot6, 0))
		...
		Search = function (slot0, slot1)
			...
			slot0.findUITimer = Timer.New(function ()
				...
				if slot2 < slot0 then
					...
					if slot4.conditionData ~= nil then
						slot0 = slot6(slot4.path, slot4.conditionData)
					else
						slot0 = slot7(slot4.path, slot4.pathIndex)
					end

					if slot0 and go(slot0).activeInHierarchy then
						slot5:Clear()
						slot4.found(slot0)
		...
		slot0.targetTimer = Timer.New(function ()
			if not IsNil(IsNil) and not IsNil(slot1) then
				...
				if slot1.image and type(slot2.image) == "table" then
					if slot2.image.isChild then
						slot2 = tf(slot0):Find(slot2.image.source)
					else
						slot2 = GameObject.Find(slot2.image.source)
					end

					slot4 = nil

					if slot2.image.isRelative then
						if slot2.image.target == "" then
							slot4 = slot1
						else
							slot4 = tf(slot1):Find(slot2.image.target)
						end
					else
						slot4 = GameObject.Find(slot2.image.target)
					end
					...
					if not IsNil(slot2) and not IsNil(slot4) then
						...
						if slot2:GetComponent(typeof(Image)) and slot6 then
							...
							if slot5.sprite and slot8 and slot7.name ~= slot8.name then
...
function pg.GuideMgr.setFinger(slot0, slot1, slot2)
	...
	if not slot2.scale or not (1 / slot2.scale) then
		slot5 = 1
	end

	slot0.fingerTF.localScale = Vector3(slot5, slot5, 1)

	if not slot2.fingerPos or not Vector3(slot2.fingerPos.posX, slot2.fingerPos.posY, 0) then
		slot6 = Vector3(slot3, -slot4, 0)
	end
...
function pg.GuideMgr.addUIEventTrigger(slot0, slot1, slot2, slot3)
	...
	if not slot4.triggerType or not slot4.triggerType[1] then
		slot8 = slot0
	end
		...
		if slot8 == slot1 then
			onToggle(slot0, slot6, function (slot0)
				if slot0 and not IsNil(slot0) then
					...
					if not slot4.triggerType[2] then
						slot3 = true
					end
