...
function pg.SystemOpenMgr.notification(slot0, slot1)
	...
	if slot1(slot1, getProxy(PlayerProxy).getData(slot2)) and not pg.MsgboxMgr.GetInstance()._go.activeSelf and slot4.story_id and slot4.story_id ~= "" and not slot0.active and not pg.StoryMgr:GetInstance():IsPlayed(slot4.story_id) then
		slot0.active = true
